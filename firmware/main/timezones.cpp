...
uint32_t getTotalTimeZones() {
   return ((sizeof(my_timezones))/sizeof(my_timezones[0]));
}

const char *getTZ(uint32_t i) {
   if(i<getTotalTimeZones()) return my_timezones[i].TZName;
   return "Etc/UTC";
}

const char *getESPTZ(uint32_t i) {
   if(i<getTotalTimeZones()) return my_timezones[i].ESPTZ;
   return "UTC0";
}

const char *getESPTZ(const char *tzname) {
   for(int i=0;i<getTotalTimeZones();++i) {
      if(strcmp(getTZ(i),tzname)==0) {
         return my_timezones[i].ESPTZ;
      }
   }
   return "UTC0";
}
